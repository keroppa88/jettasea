...
const CLOUD_CONFIGS = [
  ...
  {
    // Summer: scattered soft fair-weather clouds at every height, drifting slowly.
    cov: 0.53, soft: 0.20, scale: 0.9, stretch: 1.0, warp: 1.4,
    detail: 0.85, proj: 0.05, minY: 0.03, band: 0.12,
    flow: 0.012, drift: 0.02, shadow: 0.86, opacity: 0.92,
  },
];
const SKY = [
...
export function createClouds() {
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        if(uSummer>0.5){
          // Faint high streaks of cirrus behind the scattered summer clouds.
          vec2 cp=dir.xz/max(dir.y,0.08)*0.5;
          float streak=fbm(vec2(cp.x*0.35+cp.y*0.2,cp.y*2.4)+vec2(uDrift*0.4,0.0));
          float wisp=smoothstep(0.55,0.80,streak)*smoothstep(0.12,0.40,dir.y)*0.28;
          col=mix(vec3(0.97,0.98,1.0),col,alpha/max(alpha+wisp*(1.0-alpha),0.001));
          alpha=alpha+wisp*(1.0-alpha);
        }
  ...
  function setPalette(index) {
    ...
    mode = index === 4 ? 2 : index === 1 || index === 3 ? 1 : 0;
